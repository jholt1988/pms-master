import { PrismaClient, PackKind, CrossRiskEdge, ActionTier, IntentStatus, BuildingVintageBucket, PropertyClassBucket } from '@prisma/client';
...
async function main() {
  console.log('Starting to seed mock database data...');

  // 1. Clean existing data (optional, but highly recommended for a seed script)
  ...
  const globalKinds = [
    PackKind.GLOBAL_COUPLING_CEILINGS,
    PackKind.CROSS_RISK_GLOBAL,
    PackKind.SECONDARY_DAMAGE_GLOBAL,
    PackKind.WORKFLOW_POLICY_GLOBAL,
  ];
  ...
  for (const tenant of tenants) {
    const tenantKinds = [
      PackKind.CROSS_RISK_TENANT_EDGE_OVERLAY,
      PackKind.SHOCK_VENDOR_LEADTIME,
      PackKind.SECONDARY_DAMAGE_TENANT,
      PackKind.WORKFLOW_POLICY_TENANT,
    ];
  ...
  const edges = [CrossRiskEdge.M_to_R, CrossRiskEdge.R_to_M, CrossRiskEdge.M_to_V, CrossRiskEdge.R_to_V];

  for (const tenant of tenants) {
    // Find the overlay pack hash for this tenant
    const tenantPack = packs.find(p => p.tenantId === tenant.id && p.kind === PackKind.CROSS_RISK_TENANT_EDGE_OVERLAY);
  ...
  for (const tenant of tenants) {
    ...
    for (let i = 0; i < numIntents; i++) {
      ...
      const tier = pickRandom([ActionTier.TIER_1, ActionTier.TIER_2, ActionTier.TIER_3]);
      const status = pickRandom(Object.values(IntentStatus));
      ...
      const intent = await prisma.actionIntent.create({
        data: {
          ...
          tier,
          status,
          bundleHash: status === IntentStatus.EXECUTED || status === IntentStatus.COMPLETED ? generateHash() : null,
          governanceJson: {
            approvedBy: status === IntentStatus.APPROVED || status === IntentStatus.EXECUTED ? 'admin_user@propertyos.com' : null,
  ...
  // Also create a few global audit events
  for (let i = 0; i < 3; i++) {
    await prisma.auditEvent.create({
      data: {
        type: 'SYSTEM_STARTUP',
  ...
  for (const tenant of tenants) {
    ...
    for (let i = 0; i < numOutcomes; i++) {
      ...
      const outcome = await prisma.secondaryDamageOutcome.create({
        data: {
          tenantId: tenant.id,
          propertyId: `prop_${Math.floor(randomRange(101, 105))}`,
          unitId: `unit_${Math.floor(randomRange(1001, 1050))}`,
          ...
          vintageBucket: pickRandom(Object.values(BuildingVintageBucket)),
          classBucket: pickRandom(Object.values(PropertyClassBucket)),
