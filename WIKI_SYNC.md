...
This repository uses GitHub Actions to automatically sync wiki documentation from the `docs/guides/wiki` directory to the GitHub Wiki.

## How It Works

The workflow (`.github/workflows/sync-wiki.yml`) automatically:

1. Triggers when changes are pushed to the `master` branch that affect files in `docs/guides/wiki/`
2. Clones the GitHub Wiki repository
3. Copies all markdown files from `docs/guides/wiki/` to the wiki
...
- Copy all markdown files from `docs/guides/wiki/`
...
1. Edit the markdown files in `docs/guides/wiki/`
